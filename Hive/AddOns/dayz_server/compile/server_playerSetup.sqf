...
if (_randomSpot) then {
	...
	for [{_j=0},{_j<=100 AND _findSpot},{_j=_j+1}] do {
		...
		_findTown = nearestLocations [_point, ["NameVillage","NameCity","NameCityCapital","NameMarine"], 20000];
		_selectRandomTown = _findTown call BIS_fnc_selectRandom;
		_townPos = position _selectRandomTown;
		_position = ([_townPos,0,600,10,0,2,0] call BIS_fnc_findSafePos);
		if ((count _position >= 2) // !bad returned position
			AND {(_position distance _townPos < 500)}) then { // !ouside the disk
			_position set [2, 0];
			if (((ATLtoASL _position) select 2 > 2.5) //! player's feet too wet
			AND {({alive _x} count (_position nearEntities ["Man",1250]) == 0)}) then { // !too close from other players/zombies
				_pos = +(_position);
	...
	if (_findSpot) exitWith {
		diag_log format["%1: Error, failed to find a suitable spawn spot for player. area:%2",__FILE__, _townPos];
	};
